...
$CSVDIR = "csv";                       // directory for csv files; empty string to
// disallow csv files
...
function GetMessage($i_msg_num, $a_params = array(), $b_show_mnum = true, $b_no_errors = false) {
    ...
    if (strpos($s_text, '$') !== false) {
        ...
        $s_text = preg_replace_callback('/\$([a-z][a-z0-9_]*)/i', 'GetMessageSubstituteParam', $s_text);
...
if ($RECAPTCHA_PRIVATE_KEY !== "") {
    ...
    class reCaptchaWrapper {
        ...
        function Check($s_response, $a_values, &$s_error) {
            ...
            if (!$this->_bDone)
                $this->_Resp = recaptcha_check_answer($this->_sPrivate, $_SERVER["REMOTE_ADDR"], $a_values["recaptcha_challenge_field"], $s_response);
    ...
    function _PrepareTextSubstitute($s_value) {
        ...
        if (!IsPHPAtLeast("4.3.0"))
            ...
            for ($ii = 0; $ii < count($TEXT_SUBS); $ii++) {
                ...
                if (($n_matches = preg_match_all($TEXT_SUBS[$ii]["srch"], $s_value, $a_matches, PREG_OFFSET_CAPTURE)) !== false && $n_matches > 0) {
            ...
            $a_toks = GetTokens($s_spec);
                ...
                if (count($a_toks) > 0) {
                    ...
                    if (($n_tok = count($a_toks)) < 6 ||
                                    ...
                                    $a_toks[3] != ";" ||
                                    $a_toks[$n_tok - 1] != ")")
                                SendAlert(GetMessage(MSG_DER_FUNC_ERROR, array("SPEC" => $s_spec,
                    ...
                    if (count($a_toks) != 4 ||
                                    ...
                                    $a_toks[3] != ")")
                                SendAlert(GetMessage(MSG_DER_FUNC_ERROR, array("SPEC" => $s_spec,
                                    "MSG" => GetMessage(MSG_DER_FUNC1_FMT, array("FUNC" => $a_toks[0])))));
...
function GetNextNum($i_pad, $i_base) {
    ...
    if (($fp = @fopen($NEXT_NUM_FILE, "r+")) === false) {
        ...
        Error("next_num_file", GetMessage(MSG_NEXT_NUM_FILE, array("FILE" => $NEXT_NUM_FILE,
            "ACT" => "open",
            "ERR" => $php_errormsg)));
...
function ProcessAttributeList($a_list, &$a_attribs, &$a_errors, $a_valid_attribs = array()) {
        ...
        if ($b_filter && $b_got_filter &&
            ...
            if ($a_filter_list !== false) {
                ...
                list($s_unfiltered, $s_filtered_results) =
                        GetFilteredOutput($aFieldOrder, $aCleanedValues, $s_filter_name, $a_filter_list);
    ...
    if (($a_lines = $http_get->Read()) === FALSE) {
        ...
        switch ($i_error) {
            ...
            default:
                ...
                $s_error = GetMessage(MSG_SOCKET, array("ERRNO" => $i_sys_err,
                    "ERRSTR" => $s_sys_msg,
...
function UserError($s_error_code, $s_error_mesg, $s_extra_info = "", $a_item_list = array()) {
...
function FieldTest($s_oper, $s_fld1, $s_fld2, $a_vars, &$s_error_mesg, $s_friendly1 = "", $s_friendly2 = "") {
    ...
    switch ($s_oper) {
        ...
        case '^':       // either field but not both must be present
            ...
            if ($b_got1 || $b_got2) {
                ...
                $s_error_mesg = GetMessage(MSG_NOT_BOTH, array("ITEM1" => $s_friendly1,
                        "ITEM2" => $s_friendly2));
            ...
            $s_error_mesg = GetMessage(MSG_XOR, array("ITEM1" => $s_friendly1,
                    "ITEM2" => $s_friendly2));
        ...
        case '=':
            ...
            $b_match = !$b_match;
                ...
                $s_desc = GetMessage(MSG_IS_SAME_AS, array("ITEM1" => $s_friendly1,
                    "ITEM2" => $s_friendly2));
            }
            else
                $s_desc = GetMessage(MSG_IS_NOT_SAME_AS, array("ITEM1" => $s_friendly1,
                    "ITEM2" => $s_friendly2));
...
function AdvancedRequired($s_cond, $i_span, $a_vars, &$s_missing, &$a_missing_list) {
    ...
    switch ($s_oper) {
        ...
        list($s_fld2, $s_friendly2) = GetFriendlyName(substr($s_rem, $i_span));
            ...
            if (!FieldTest($s_oper, $s_fld1, $s_fld2, $a_vars, $s_error_mesg, $s_friendly1, $s_friendly2)) {
...
function CheckConditions($m_conditions, $a_vars, &$s_missing, &$a_missing_list, $m_id = false) {
    ...
    for ($ii = 0; $ii < count($a_list); $ii++) {
        ...
        if (count($a_components) < 5) {
            ...
            SendAlert(GetMessage(MSG_COND_INVALID, array("FLD" => $s_fld_name, "COND" => $s_cond,
                "SEP" => $s_int_sep)));
        ...
        switch ($a_components[0]) {
            ...
            case "TEST":
                ...
                if (count($a_components) > 5) {
                    ...
                    SendAlert(GetMessage(MSG_COND_TEST_LONG, array("FLD" => $s_fld_name, "COND" => $s_cond,
                        "SEP" => $s_list_sep)));
            ...
            case "IF":
                ...
                if (count($a_components) < 6) {
                    ...
                    SendAlert(GetMessage(MSG_COND_IF_SHORT, array("FLD" => $s_fld_name, "COND" => $s_cond,
                        "SEP" => $s_int_sep)));
                ...
                if (count($a_components) > 7) {
                    ...
                    SendAlert(GetMessage(MSG_COND_IF_LONG, array("FLD" => $s_fld_name, "COND" => $s_cond,
                        "SEP" => $s_list_sep)));
            ...
            default:
                ...
                SendAlert(GetMessage(MSG_COND_UNK, array("FLD" => $s_fld_name, "COND" => $s_cond,
                    "CMD" => $a_components[0])));
...
function SocketFilter($filter, $a_filter_info, $m_data) {
    ...
    if ($f_sock === false) {
        ...
        Error("filter_connect", GetMessage(MSG_FILTER_CONNECT, array(
            ...
            "SITE" => $s_site,
            "ERRNUM" => $i_errno,
            "ERRSTR" => "$s_errstr (" . CheckString($php_errormsg) . ")")), false, false);
    ...
    foreach ($a_params as $m_var) {
        ...
        if (is_array($m_var)) {
            ...
            if (!isset($m_var["name"])) {
                ...
                Error("bad_filter", GetMessage(MSG_FILTER_PARAM, array("FILTER" => $filter,
                    "NUM" => $i_count,
...
@ $fp = fopen($m_var["file"], "r");
            ...
            if ($fp === false) {
                ...
                Error("filter_error", GetMessage(MSG_FILTER_OPEN_FILE, array("FILTER" => $filter,
                    "FILE" => $m_var["file"],
            ...
            while (!feof($fp)) {
                ...
                if (($s_line = fgets($fp, 2048)) === false)
                    if (feof($fp))
                        ...
                        Error("filter_error", GetMessage(MSG_FILTER_FILE_ERROR, array("FILTER" => $filter,
                            "FILE" => $m_var["file"],
                            "ERROR" => CheckString($php_errormsg),
    ...
    while (!feof($f_sock)) {
        ...
        if (($s_line = fgets($f_sock, 2048)) === false)
            if (feof($f_sock))
                ...
                Error("filter_failed", GetMessage(MSG_FILTER_READ_ERROR, array("FILTER" => $filter,
                    "ERROR" => CheckString($php_errormsg))), false, false);
    ...
    if (!$b_ok) {
        ...
        Error("filter_failed", GetMessage(MSG_FILTER_NOT_OK, array("FILTER" => $filter,
...
function Filter($filter, $m_data) {
    ...
    elseif ($FILTERS[$filter] == "csv")
        ...
        if ($dirname != "" && $dirname != "." && !chdir($dirname)) {
            ...
            Error("chdir_filter", GetMessage(MSG_FILTER_CHDIR, array("DIR" => $dirname, "FILTER" => $filter,
                "ERROR" => CheckString($php_errormsg))), false, false);
        ...
        if ($pipe === false) {
            ...
            Error("filter_not_found", GetMessage(MSG_FILTER_NOTFOUND, array("CMD" => $cmd, "FILTER" => $filter,
        ...
        if (($i_st = pclose($pipe)) != 0) {
            ...
            Error("filter_failed", GetMessage(MSG_FILTER_ERROR, array("FILTER" => $filter,
                "ERROR" => $s_sv_err,
...
class CSVFormat {
    ...
    function CSVFormat($c_sep = ',', $c_quote = '"', $c_int_sep = ';', $s_esc_policy = "backslash", $s_clean_func = NULL) {
    ...
    function _Escape($m_value) {
        ...
        switch ($this->_sEscPolicy) {
            ...
            case "b":
                ...
                $m_value = str_replace($this->_cQuote, "\\" . $this->_cQuote, $m_value);
            ...
            case "d":
                ...
                $m_value = str_replace($this->_cQuote, $this->_cQuote . $this->_cQuote, $m_value);
...
function BuiltinFilterCSV() {
    ...
    if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols)) {
        ...
        if (!isset($s_cols) || empty($s_cols) || !is_string($s_cols)) {
            ...
            $a_column_list = array_merge($a_column_list, array_keys($aRawDataValues));
    ...
    if (!isset($m_temp))
        $csv_format->SetCleanFunc(create_function('$m_value', 'return CleanValue($m_value);'));
...
function ProcessTemplate($s_template, &$a_lines, $a_values, $s_missing = NULL, $s_subs_func = 'SubstituteValue') {
    ...
    return (DoProcessTemplate($TEMPLATEDIR, $TEMPLATEURL, $s_template, $a_lines, $a_values, $s_missing, $s_subs_func));
}
...
function OutputTemplate($s_template, $a_values) {
    ...
    if (!ProcessTemplate($s_template, $a_lines, $a_values, "", 'SubstituteValueForPage'))
        Error("template_failed", GetMessage(MSG_TEMPLATE_FAILED, array("NAME" => $s_template)), false, false);
...
function ProcessReturnToForm($s_url, $a_values, $a_strip = array()) {
    ...
    if ($s_form_buf === false)
        Error("invalid_url", GetMessage(MSG_OPEN_URL, array("URL" => $s_form_url,
...
function OutputMultiFormTemplate($s_template, $a_values) {
    ...
    if (!ProcessMultiFormTemplate($s_template, $a_values, $a_lines))
        Error("multi_form_failed", GetMessage(MSG_MULTIFORM_FAILED, array("NAME" => $s_template)), false, false);
...
function HTMLMail(&$a_lines, &$a_headers, $s_body, $s_template, $s_missing, $s_filter, $s_boundary, $a_raw_fields, $b_no_plain, $b_process_template) {
    ...
    if ($b_no_plain) {
        ...
        $a_pre_lines = explode("\n", GetMessage(MSG_MIME_HTML, array("NAME" => $s_template)));
...
function SaveFileInRepository(&$a_file_spec) {
    ...
    // this exits
        ...
        UserError("upload_size", GetMessage(MSG_FILE_UPLOAD_SIZE, array("NAME" => $a_file_spec["name"],
...
function SaveAllFilesToRepository() {
    ...
    foreach ($aFileVars as $m_file_key => $a_upload) {
        ...
        if (!IsUploadedFile($a_upload)) {
            ...
            SendAlert(GetMessage(MSG_FILE_UPLOAD_ATTACK, array("NAME" => $a_upload["name"],
...
function MakeMimeMail(&$s_body, &$a_headers, $a_raw_fields, $s_template = "", $s_missing = NULL, $b_no_plain = false, $s_filter = "", $a_file_vars = array(), $a_attach_spec = array(), $b_process_template = true) {
    ...
    if (isset($s_template) && !empty($s_template)) {
        ...
        if (!IsPHPAtLeast("4.0.5")) {
            ...
            SendAlert(GetMessage(MSG_PHP_HTML_TEMPLATES, array("PHPVERS" => implode(".", $aPHPVERSION))));
    ...
    if (count($a_file_vars) > 0) {
        ...
        if (!IsPHPAtLeast("4.0.3")) {
            ...
            SendAlert(GetMessage(MSG_PHP_FILE_UPLOADS, array("PHPVERS" => implode(".", $aPHPVERSION))));
    ...
    if ($b_att) {
        ...
        if ($b_html) {
            ...
            if (!HTMLMail($a_lines, $a_local_headers, $s_body, $s_template, $s_missing, ($b_got_filter) ? $s_filter : "", $s_body_boundary, $a_raw_fields, $b_no_plain, $b_process_template))
                return (false);
        ...
        if (FILEUPLOADS && ($FILE_REPOSITORY === "" || IsMailOptionSet("AlwaysEmailFiles")))
            foreach ($a_file_vars as $m_file_key => $a_upload) {
                ...
                if (MAX_FILE_UPLOAD_SIZE != 0 &&
                    ...
                    UserError("upload_size", GetMessage(MSG_FILE_UPLOAD_SIZE, array("NAME" => $a_upload["name"],
                        "SIZE" => $a_upload["size"],
                        "MAX" => MAX_FILE_UPLOAD_SIZE)));



                if (!AttachFile($a_new, $s_att_boundary, $a_upload, $s_charset, ($FILE_REPOSITORY === "") ? true : false))
                    return (false);
            }
        ...
        if (isset($a_attach_spec["Data"])) {
            ...
            if (!AttachFile($a_new, $s_att_boundary, $a_file_spec, isset($a_attach_spec["CharSet"]) ?
    ...
    elseif ($b_html) {
        ...
        if (!HTMLMail($a_new, $a_headers, $s_body, $s_template, $s_missing, ($b_got_filter) ? $s_filter : "", $s_body_boundary, $a_raw_fields, $b_no_plain, $b_process_template))
            return (false);
    }
                ...
                if ($b_got_filter) {
                    ...
                    if ($b_limited_filter)
                        list($s_discard, $s_filtered_results) =
                                GetFilteredOutput($a_fld_order, $a_clean_fields, $s_filter, $a_filter_list);
    ...
    if (count($a_keys) == 1 && is_string($a_raw_fields[$a_keys[0]]) &&
        ...
        if ($b_mime_mail) {
            ...
            list($s_results, $s_unfiltered_results, $s_filtered_results) =
                    MakePlainEmail($a_fld_order, $a_clean_fields, $s_to, $s_cc, $s_bcc, $a_raw_fields, $s_filter, $a_filter_list);
...
function CheckConfig() {
    ...
    if (in_array("TARGET_EMAIL", $CONFIG_CHECK)) {
        ...
        for ($ii = 0; $ii < count($TARGET_EMAIL); $ii++) {
            ...
            if (substr($s_pattern, 0, 1) != '^')
                $a_mesgs[] = GetMessage(MSG_TARG_EMAIL_PAT_START, array("PAT" => $s_pattern));



            if (substr($s_pattern, -1) != '$')
                $a_mesgs[] = GetMessage(MSG_TARG_EMAIL_PAT_END, array("PAT" => $s_pattern));
        }
    }
    ...
    if (count($a_mesgs) > 0)
        SendAlert(GetMessage(MSG_CONFIG_WARN, array("MESGS" => implode("\n", $a_mesgs))), false, true);
}
...
if (isset($aGetVars["testalert"]) && $aGetVars["testalert"] == 1) {
    ...
    $sAlert = GetMessage(MSG_ALERT, array("LANG" => $sLangID,
...
function GetMultiValues($a_form_list, $i_form_index, $a_order = array(), $a_clean = array(), $a_raw_data = array(), $a_all_data = array(), $a_file_data = array()) {
...
function MultiFormReturn($i_return_to) {
    ...
    if (!IsSetSession("FormList") ||
        ...
        Error("cannot_return", GetMessage(MSG_CANNOT_RETURN, array("TO" => $i_return_to,
            "TOPINDEX" => (
    ...
    foreach ($a_rec_flds as $s_fld)
        if (isset($a_fields[$s_fld])) {
            ...
            if (($i_mime = strpos($s_data, "mime-version")) !== false ||
                ...
                $s_info = GetMessage(MSG_ATTACK_MIME_INFO, array("FLD" => $s_fld,
    ...
    foreach ($a_fields as $s_fld => $m_value) {
        ...
        if (isset($m_value) && !FieldManager::IsEmpty($m_value)) {
            ...
            foreach ($m_value as $s_data) {
                ...
                $n_match = AttackDetectionFindJunk($s_data, ATTACK_DETECTION_JUNK_CONSONANTS, ATTACK_DETECTION_JUNK_CONSEC_CONSONANTS, $a_match_cons);
                ...
                $n_match = AttackDetectionFindJunk($s_data, ATTACK_DETECTION_JUNK_VOWELS, ATTACK_DETECTION_JUNK_CONSEC_VOWELS, $a_match_vow);
                ...
                if ($n_count >= ATTACK_DETECTION_JUNK_TRIGGER) {
                    ...
                    $s_user_info = GetMessage(MSG_USER_ATTACK_JUNK, array("INPUT" => implode(", ", $a_user_matches)), false);
                    ...
                    $s_info = GetMessage(MSG_ATTACK_JUNK_INFO, array("FLD" => $s_fld,
    ...
    foreach ($ATTACK_DETECTION_DUPS as $s_fld)
        if (isset($a_fields[$s_fld]) &&
            ...
            if (isset($a_data_map[$s_data])) {
                ...
                $s_info = GetMessage(MSG_ATTACK_DUP_INFO, array("FLD1" => $a_data_map[$s_data],
                    "FLD2" => $s_fld), false);
...
function DetectSpecialsAttack($a_fields, &$s_attack, &$s_info, &$s_user_info) {
    ...
    foreach ($ATTACK_DETECTION_SPECIALS_ONLY_EMAIL as $s_fld) {
        ...
        if (isset($a_fields[$s_fld]) &&
            ...
            if (preg_match("/^\b[-a-z0-9._%]+@[-.%_a-z0-9]+\.[a-z]{2,9}\b$/i", $s_data) === 1) {
                ...
                $s_info = GetMessage(MSG_ATTACK_SPEC_INFO, array("FLD" => $s_fld), false);
    ...
    foreach ($ATTACK_DETECTION_SPECIALS_ANY_EMAIL as $s_fld) {
        ...
        if (isset($a_fields[$s_fld]) &&
            ...
            if (preg_match("/\b[-a-z0-9._%]+@[-.%_a-z0-9]+\.[a-z]{2,9}\b/i", $s_data) > 0) {
...
function DetectManyURLsAttack($a_fields, &$s_attack, &$s_info, &$s_user_info) {
        ...
        if (isset($s_data) &&
            ...
            if (ATTACK_DETECTION_MANY_URLS > 0)
                if ($n_match >= ATTACK_DETECTION_MANY_URLS) {
                    ...
                    $s_info = GetMessage(MSG_ATTACK_MANYURL_INFO, array("FLD" => $s_fld, "NUM" => ($n_match)), false);
    ...
    if (ATTACK_DETECTION_MANY_URL_FIELDS > 0)
        if (count($a_fld_names) >= ATTACK_DETECTION_MANY_URL_FIELDS) {
            ...
            $s_info = GetMessage(MSG_ATTACK_MANYFIELDS_INFO, array("FLDS" => implode(",", $a_fld_names),
...
function DetectAttacks($a_fields) {
    ...
    if ($b_attacked) {
        ...
        if (ALERT_ON_ATTACK_DETECTION)
            SendAlert(GetMessage(MSG_ATTACK_DETECTED, array("ATTACK" => $s_attack,
                "INFO" => $s_info,)), false);
    ...
    foreach ($a_revcap_spec as $s_fld_name => $s_value) {
        ...
        if ($s_value === "") {
            ...
            if (isset($a_form_data[$s_fld_name]) &&
                ...
                $s_info .= "\n" . GetMessage(MSG_ATTACK_REV_CAP_INFO, array("FLD" => $s_fld_name,
...
if (isset($aGetVars["return"]) && is_numeric($aGetVars["return"])) {
    ...
    if ($bAdvTemplates) {
        ...
        if (!include_once("$MODULEDIR/$FMCOMPUTE"))
            Error("load_fmcompute", GetMessage(MSG_LOAD_FMCOMPUTE, array("FILE" => "$MODULEDIR/$FMCOMPUTE",
...
if (IsSetSession("FormList") && $SPECIAL_VALUES["multi_start"] != 1) {
    ...
    list($aFieldOrder, $aCleanedValues,
            ...
            GetSession("FormList"), GetSession("FormIndex"), $aFieldOrder, $aCleanedValues, $aRawDataValues, $aAllRawValues, $aFileVars);
}
...
if ($SPECIAL_VALUES["file_names"] !== "")
    list($aFieldOrder, $aCleanedValues, $aRawDataValues, $aAllRawValues, $aFileVars) =
            SetFileNames($SPECIAL_VALUES["file_names"], $aFieldOrder, $aCleanedValues, $aRawDataValues, $aAllRawValues, $aFileVars);
...
if (!empty($SPECIAL_VALUES["fmcompute"]) || $bAdvTemplates) {
    ...
    function FM_CallFunction($s_func, $a_params, &$m_return, &$s_mesg, &$a_debug, &$a_alerts) {
        ...
        switch ($s_func) {
            ...
            case "FMFatalError":
                ...
                if (count($a_params) < 3)
                    Error("fmcompute_call", GetMessage(MSG_CALL_PARAM_COUNT, array("FUNC" => $s_func,
                        "COUNT" => count($a_params))), false, false);
                else
            ...
            case "FMNextNum":
                ...
                if (count($a_params) != 2) {
                    ...
                    if ($i_base < 2 || $i_base > 36)
                        Error("fmcompute_call", GetMessage(MSG_CALL_INVALID_PARAM, array("FUNC" => $s_func,
                            "PARAM" => 2,
                            "CORRECT" => 2. . .36)), false, false);
    ...
    function RegisterFormMailFunctions(&$fmc) {
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "void", "FMFatalError", array("string", "bool", "bool"), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMFatalError",
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "void", "FMFatalUserError", array("string"), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMFatalUserError",
                "ERROR" => $s_msg)), false, false);
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "void", "FMUserError", array("string"), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMUserError",
                "ERROR" => $s_msg)), false, false);
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "bool", "FMSaveAllFilesToRepository", array(), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMSaveAllFilesToRepository",
                "ERROR" => $s_msg)), false, false);
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "bool", "FMDeleteFileFromRepository", array("string"), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMDeleteFileFromRepository",
                "ERROR" => $s_msg)), false, false);
        ...
        if (($s_msg = $fmc->RegisterExternalFunction("PHP", "string", "FMNextNum", array("int", "int"), "FM_CallFunction")) !== true)
            Error("fmcompute_reg", GetMessage(MSG_REG_FMCOMPUTE, array("FUNC" => "FMNextNum",
                "ERROR" => $s_msg)), false, false);
    }
    ...
    if (!empty($SPECIAL_VALUES["fmcompute"])) {
        ...
        if (!empty($GEOIP_LIC)) {
            ...
            if (!include_once("$MODULEDIR/$FMGEOIP"))
                Error("load_module", GetMessage(MSG_LOAD_MODULE, array("FILE" => "$MODULEDIR/$FMGEOIP",
                    "ERROR" => $php_errormsg)), false, false);
            ...
            if (!$GeoIP->RegisterModule($FMCalc))
                Error("reg_module", GetMessage(MSG_REGISTER_MODULE, array("NAME" => "FMGeoIP",
...
if (!empty($SPECIAL_VALUES["fmmodules"])) {
    ...
    foreach ($aModuleList as $sModule)
        if (!include_once("$MODULEDIR/$sModule"))
            Error("load_module", GetMessage(MSG_LOAD_MODULE, array("FILE" => "$MODULEDIR/$sModule",
                "ERROR" => $php_errormsg)), false, false);
}
...
if (!empty($SPECIAL_VALUES["fmcompute"])) {
    ...
    function Load($s_code) {
        ...
        if ($FMCalc->Parse($s_code, $a_mesgs) === false) {
            ...
            Error("fmcompute_parse", GetMessage(MSG_COMP_PARSE, array("CODE" => AddLineNumbers($s_code),
    ...
    function SendComputeAlerts() {
        ...
        if (count($a_alerts) > 0)
            SendAlert(GetMessage(MSG_COMP_ALERT, array("ALERTS" => implode("\n", StripHTML($a_alerts)))));
        ...
        if (count($a_debug) > 0)
            SendAlert(GetMessage(MSG_COMP_DEBUG, array("DEBUG" => implode("\n", StripHTML($a_debug)))));
    }
    ...
    function Compute(&$a_field_order, &$a_cleaned_values, &$a_raw_data_values, &$a_values) {
        ...
        if (($a_flds = $FMCalc->Execute($a_mesgs)) !== false) {
            ...
            foreach ($a_flds as $s_name => $s_value) {
                ...
                ProcessField($s_name, $s_value, $a_field_order, $a_cleaned_values, $a_raw_data_values);
            }



            if (count($FM_UserErrors) > 0)
                UserError("fmcompute_usererrors", GetMessage(MSG_USER_ERRORS), "", $FM_UserErrors);
        }
            ...
            Error("fmcompute_exec", GetMessage(MSG_COMP_EXEC, array("ERRORS" => implode("\n", $a_mesgs))), false, false);
        }
    }
        ...
        foreach ($a_file_vars as $s_fld_name => $a_file_spec) {
            ...
            if (IsUploadedFile($a_file_spec)) {
                ...
                if (isset($a_file_spec["new_name"])) {
                    ...
                    $FMCalc->RegisterExternalData("PHP", "string", "name_of_" . $s_fld_name, "c", $a_file_spec["new_name"]);
                }
    ...
    if (!empty($sCRM)) {
        ...
        if (!SendToCRM($sCRM, $aCRMReturnData)) {
            ...
            if (IsCRMOptionSet("ErrorOnFail"))
                Error("crm_failed", GetMessage(MSG_CRM_FAILURE, array("URL" => $sCRM,
                    "DATA" => GetObjectAsString($aCRMReturnData))), false, false);
        }
        else
